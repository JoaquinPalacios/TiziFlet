import Link from 'next/link';
import Image from 'next/image';
import { Navbar as NavbarBootstrap, Nav } from "react-bootstrap";
import { FaQuoteLeft, FaQuoteRight } from "react-icons/fa";
import { BiPhoneCall } from "react-icons/bi";

const Navbar = () => {
    return ( 
        <NavbarBootstrap className='my-0' expand="lg">
            <div className='col-2 col-lg-1 logo ms-sm-3 me-0 pe-0'>
                <Image alt='logo' src='/images/logo.png' width='2000px' height='2000px' />
            </div>
            <div className='col-6 d-block d-lg-none d-flex justify-content-center m-auto'>
                <span className='phone'>
                    <BiPhoneCall size='2em' />
                    <a href='tel:+542235894137'>2235-894-137</a>
                </span>
            </div> 
            <NavbarBootstrap.Toggle aria-controls="responsive-navbar-nav" className="me-3" />
            <NavbarBootstrap.Collapse className='col-4 col-lg-11' id="responsive-navbar-nav">
                <Nav className='col-4 d-flex d-none d-xl-block fs-5 fst-italic justify-content-start align-self-end ms-0 ps-0'>
                    <FaQuoteLeft size='1.2em' className='pb-3' />Mudanzas en Mar del Plata y alrededores<FaQuoteRight size='1.2em' className='pb-3' />
                </Nav> 
                <div className='col-4 col-xl-3 d-none d-lg-block m-auto'>
                    <span className='phone ms-xl-2'>                        
                        <a href='tel:+542235894137' className='text-decoration-none text-white fw-bold'><BiPhoneCall size='1.5em' className='pe-1' />2235-894-137</a>
                    </span>
                </div>           
                <Nav className='col-8 col-lg-6 col-xl-5 d-flex justify-content-end align-self-center me-2'>
                    <Link href="/"><a className='fs-5 mx-3 my-2 my-lg-0 text-decoration-none text-muted'>Home</a></Link>
                    <Link href="/about"><a className='fs-5 mx-3 my-2 my-lg-0 text-decoration-none text-muted'>Quienes Somos</a></Link>
                    <Link href="/contacto"><a className='fs-5 ms-3 me-xl-2 my-2 my-lg-0 text-decoration-none text-muted'>Contacto</a></Link>
                </Nav>
            </NavbarBootstrap.Collapse>              
        </NavbarBootstrap>
     );
}
 
export default Navbar;